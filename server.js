...
        "CẢNH BÁO KHẨN CẤP!": "🚨🚨 CẢNH BÁO KHẨN CẤP!" // (Từ file NÂNG CAO)
    };
...
        "CẢNH BÁO KHẨN CẤP!": "🚨 NƯỚC DÂNG BẤT THƯỜNG! NGUY CƠ LŨ QUÉT CAO! DI CHUYỂN ĐẾN NƠI AN TOÀN NGAY LẬP TỨC!" // (Từ file NÂNG CAO)
    };
...
        if (!token) return res.status(400).json({ error: 'Missing token' });
        ...
        // (Logic này đúng)
        // Khi chạy trên Cloud, 'pool' là CSDL Cloud.
        // Khi chạy ở Local, App không gọi API này.
        if (pool) {
...
        } else if (appState.lastSensorData.mucNuocB && (mucNuocB - appState.lastSensorData.mucNuocB) > ABSOLUTE_RISE_THRESHOLD) {
            console.warn(`📈 Nước dâng đột ngột! Tăng: ${(mucNuocB - appState.lastSensorData.mucNuocB).toFixed(1)} cm`);
            await sendPushNotificationInternal("🔶 Cảnh báo Lũ Cao", `Nước dâng đột ngột, tăng ${(mucNuocB - appState.lastSensorData.mucNuocB).toFixed(1)} cm.`);
            warningTriggered = true;
        } else if (mucNuocB > HIGH_WATER_LEVEL_THRESHOLD && appState.lastSentAIStatus !== "Cảnh báo Cao!") {
...
        console.log(`🔄 [FCM Sync - Cloud] Đã đồng bộ ${appState.fcmTokens.length} token.`);
    } catch (err) {
        console.error("❌ Lỗi đồng bộ FCM token (Cloud):", err.message);
    }
}
