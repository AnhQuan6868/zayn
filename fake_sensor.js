...
// SỬA DÒNG NÀY (Nếu bạn đang chạy server trên Railway)
// const SERVER_URL = 'http://localhost:3000/update'; // URL Local
...
// Thời gian (steps)
const RAIN_DURATION_STEPS = 10; // Mưa 10 bước (20s) TRƯỚC KHI nước B dâng
...
let isRaining = false; // <-- BIẾN MỚI: True/False
let b_history_queue = [];
...
function generateRealisticData() {
    ...
    switch (currentState) {
        case 'RAINING':
            // (Không cần làm gì thêm, isRaining đã là true)
        ...
        case 'FALLING':
            ...
            if (fall_remaining > 0) {
                ...
                // Khi nước rút gần xong, thì mới TẮT MƯA
                isRaining = false;
            }
            if (++stateStepCounter >= FLOOD_FALL_TIME) {
                 currentState = 'NORMAL'; stateStepCounter = 0;
                 dangerCountdown = 0; isRaining = false; // Đảm bảo mưa tắt hẳn
            }
            break;
        default: // NORMAL
            currentA += (Math.random() - 0.5) * 0.5;
            currentA -= (currentA - BASE_MUC_NUOC_A) * 0.1;
            currentB += (Math.random() - 0.5) * 0.5;
            currentLuuLuong += (Math.random() - 0.5) * 0.2;
            currentB -= (currentB - BASE_MUC_NUOC_B) * 0.1;
            currentLuuLuong -= (currentLuuLuong - BASE_LUU_LUONG) * 0.1;
            isRaining = false; // Không mưa
            break;
    }
